...
async function getCached(addr) {
  ...
  if (m && Date.now() - m.at < TTL_MS) return { ...m, layer: "memory" };
  if (col) {
    const d = await col.findOne({ _id: addr }).catch(() => null);
    if (d && Date.now() - d.at < TTL_MS) { mem.set(addr, { at: d.at, report: d.report }); return { ...d, layer: "mongo" }; }
  }
  return null;
}
async function putCached(addr, report) {
  const doc = { at: Date.now(), report };
  mem.set(addr, doc);
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (!/^0x[0-9a-f]{40}$/.test(addr)) return send(400, { error: "address must be a 0x…40-hex address" });
    scans++;
